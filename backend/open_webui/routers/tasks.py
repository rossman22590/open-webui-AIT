...
async def generate_title(
    ...
    if model_id not in models:
        # Return a friendly 404 right away
    ...
    try:
        ...
        # Intercept "Model not found"
        if str(e) == "Model not found":
            log.warning(f"[title/completions] {user.id}: {e}")
            return JSONResponse(status_code=404, content={"detail": "Model not found"})
...
async def generate_chat_tags(
    ...
    content = tags_generation_template(template, form_data["messages"], {"name": user.name})
    ...
    try:
        ...
        if str(e) == "Model not found":
            log.warning(f"[tags/completions] {user.id}: {e}")
            return JSONResponse(status_code=404, content={"detail": "Model not found"})
...
async def generate_image_prompt(
    ...
    try:
        ...
        if str(e) == "Model not found":
            log.warning(f"[image_prompt/completions] {user.id}: {e}")
            return JSONResponse(status_code=404, content={"detail": "Model not found"})
        log.error("Exception occurred", exc_info=True)
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def generate_queries(
    ...
    log.debug(
        f"generating {type_} queries using model {task_model_id} for user {user.email}"
    )
    ...
    content = query_generation_template(template, form_data["messages"], {"name": user.name})
    ...
    try:
        ...
        if str(e) == "Model not found":
            log.warning(f"[queries/completions] {user.id}: {e}")
            return JSONResponse(status_code=404, content={"detail": "Model not found"})
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def generate_autocompletion(
    ...
    if request.app.state.config.AUTOCOMPLETE_GENERATION_INPUT_MAX_LENGTH > 0:
        if len(prompt) > request.app.state.config.AUTOCOMPLETE_GENERATION_INPUT_MAX_LENGTH:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=(
                    f"Input prompt exceeds maximum length of "
    ...
    content = autocomplete_generation_template(
        template, prompt, messages, type_, {"name": user.name}
    )
    ...
    try:
        ...
        if str(e) == "Model not found":
            log.warning(f"[auto/completions] {user.id}: {e}")
            return JSONResponse(status_code=404, content={"detail": "Model not found"})
        log.error(f"Error generating chat completion: {e}", exc_info=True)
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def generate_emoji(
    ...
    try:
        ...
        if str(e) == "Model not found":
            log.warning(f"[emoji/completions] {user.id}: {e}")
            return JSONResponse(status_code=404, content={"detail": "Model not found"})
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"detail": str(e)},
...
async def generate_moa_response(
    ...
    try:
        ...
        if str(e) == "Model not found":
            log.warning(f"[moa/completions] {user.id}: {e}")
            return JSONResponse(status_code=404, content={"detail": "Model not found"})
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"detail": str(e)},
